...
void Scheme::startWork3()
{
    // Check hold

    double sp = 0.0; // Set point temperature
    double ct = _mainData.value("temperature").toDouble(); // Current temperature

    auto timing = PhpAPI::instance()->timing();
    auto relay  = Relay::instance();
    auto relaysConfig = relay->relays();

    auto curentSysMode = mCurentSysMode;
    auto realSysMode = mRealSysMode;
        ...
        case STHERM::CoolingType::CoolingOnly: {
            if (timing->s2hold) {
                if (sp - ct >= 1) {
                    ...
                    if (timing->uptime.elapsed() / 60000 >= 120 && !timing->alerts) {
                        emit alert();
                        timing->alerts = true;
                    }
                }
                ...
                if(ct - sp <= 1.9) {
                    // Turn off stage 2 and keep stage 1 run: Y2 = 0; Y1 and G = 1
                    relaysConfig.y2 = STHERM::OFF;
                    relay->coolingStage1();
                    emit changeBacklight(QVariantList{0, 128, 255, STHERM::LedEffect::LED_FADE});
                    timing->s1uptime.invalidate();// = 0;
                ...
                } else if (/*std::difftime(timing->uptime, current_timestamp())*/timing->uptime.elapsed() / 60000 >= 120 && !timing->alerts) {
                    emit alert();
                    timing->alerts = true;
                    return;
                }
            ...
            if (sp - ct >= 1) {
                relay->setAllOff();
                return;
            } else if (relaysConfig.y2 == STHERM::RelayMode::ON) {
                if (ct - sp > 2.9 || /*std::difftime(timing->s1uptime, current_timestamp()) */ timing->s1uptime.elapsed() / 60000 >= 40) {

                    if (std::difftime(timing->s2Offtime, current_timestamp()) / 60 >= 2) {
                        emit changeBacklight(QVariantList{0, 128, 255, STHERM::LedEffect::LED_FADE});
                        relay->coolingStage2();
//                        timing->s2hold = true;
            ...
            } else if (!timing->alerts && /*std::difftime(timing->uptime, current_timestamp())*/ timing->uptime.elapsed() / 60000 >= 120) {
                emit alert();
                timing->alerts = true;
    ...
    case STHERM::SystemMode::Heating: {
        switch (mDeviceType) {
        case STHERM::CoolingType::HeatPump: {
            if(mCurentSysMode == STHERM::Emergency && ct < 40.0 - 3.5) {
                relay->emergencyHeating2();
                return;
            } else if (mCurentSysMode == STHERM::Emergency && ct < 60) {
                return;

            }

            if (mCurentSysMode == STHERM::Emergency && ct >= 60) {
                relay->turnOffEmergencyHeating();
                mCurentSysMode = STHERM::Heating;
                return;
            }
            ...
            if (timing->s2hold) {
                ...
                } else if (!timing->alerts && std::difftime(timing->uptime, current_timestamp()) / 60 >= 120) {
                    emit alert();
                    timing->alerts = true;
                }
            ...
            if (relaysConfig.y2 == STHERM::RelayMode::ON && ct - sp <= 1.9) {
                if (sp - ct >= 2.9 || /*std::difftime(timing->s1uptime, current_timestamp())*/ timing->s1uptime.elapsed()/ 60000 >= 40) {
                    if (std::difftime(timing->s2Offtime, current_timestamp()) / 60 > 2) {
                        relay->heatingStage2();
                        // 5 secs
                        emit changeBacklight(QVariantList{255, 68, 0, STHERM::LedEffect::LED_FADE});
            ...
            if (ct < sp) {
                if (ct < 40.0) {
                    ...
                    // untile the end of emergency mode.
                    emit changeBacklight(QVariantList{255, 0, 0, STHERM::LedEffect::LED_BLINK});
                    mCurentSysMode = STHERM::Emergency;
...
//                if (relaysConfig.y2 == STHERM::RelayMode::ON) { // correct
//                    if (sp - ct >= 2.9) {
...
//                }
        ...
        case STHERM::CoolingType::Conventional: {
            if (timing->s3hold) {
                ...
                if (sp - ct < 4.9) {
                    relaysConfig.w3 = STHERM::RelayMode::OFF;
                    relay->heatingStage3();
                    // 5 secs
                    emit changeBacklight(QVariantList{255, 68, 0, STHERM::LedEffect::LED_FADE});
                    QTimer timer;
                    timer.start();
                    timing->s1uptime.invalidate();
                    timing->s2uptime.invalidate();// = 0;
                    timing->s3hold = false;

                    return;
                } else if (!timing->alerts && timing->uptime.elapsed()/*std::difftime(timing->uptime, current_timestamp())*/ / 60000 >= 120) {
                    emit alert();
                    timing->alerts = true;
                    return;
                } else {
            ...
            if (timing->s2hold) {
                if (ct - sp >= 1.0) {
                    ...
                    if (/*std::difftime(timing->s2uptime, current_timestamp())*/timing->s2uptime.elapsed() / 60000 >= 10) {
                        relay->heatingStage3();
                        // 5 secs
                        emit changeBacklight(QVariantList{255, 68, 0, STHERM::LedEffect::LED_FADE});
                        timing->s2hold = true;
                ...
                } if (!timing->alerts && timing->uptime.elapsed()/*std::difftime(timing->uptime, current_timestamp())*/ / 60000 >= 120) {
                    emit alert();
                    timing->alerts = true;
                    return;
                } else {
                ...
                if (sp - ct < 8) {
                    if (sp - ct < 1.9) {
                        // turn of stage 2, keep run stage 1
                        relaysConfig.w2 = STHERM::RelayMode::OFF;
                        relaysConfig.w3 = STHERM::RelayMode::OFF;
                        relay->heatingStage1();
                        // 5 secs
                        emit changeBacklight(QVariantList{255, 68, 0, STHERM::LedEffect::LED_FADE});
                    ...
                    if (relaysConfig.w3 == STHERM::RelayMode::ON) {
                        if (/*std::difftime(timing->s2uptime, current_timestamp())*/ timing->s2uptime.elapsed() / 60000 >= 10 || sp - ct >= 5.9) {
                            relay->heatingStage3();
                            // 5 secs
                            emit changeBacklight(QVariantList{255, 68, 0, STHERM::LedEffect::LED_FADE});
                            timing->s2hold = false;
            ...
            if (sp - ct >= 1.9) {
                ...
                if (relaysConfig.w2 == STHERM::RelayMode::ON) {
                    if (sp - ct >= 2.9 || /*std::difftime(timing->s1uptime, current_timestamp())*/ timing->s1uptime.elapsed() / 60000 >= 10) {
                        relay->heatingStage2();
                        //5 sec
                        emit changeBacklight(QVariantList{255, 68, 0, STHERM::LedEffect::LED_FADE});
                        timing->s2uptime.invalidate();// = 0;
